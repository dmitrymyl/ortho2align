...
def _anchor(query_genes, query_anchors, query_genome_filename,
    ...
    for query_gene in query_genes:
        ...
        synteny_list = GenomicRangesList(syntenies,
                                         sequence_file_path=subject_genome_filename)
        query_gene.relations['lifted'] = synteny_list
    ...
    query_prepared_genes = GenomicRangesList(query_prepared,
                                             sequence_file_path=query_genome_filename)
    query_unliftable_genes = GenomicRangesList(query_unliftable,
                                               sequence_file_path=query_genome_filename)
    return query_prepared_genes, query_unliftable_genes
...
def get_alignments(query_genes,
    ...
    with CmdProgressBar(cmd_hints,
        ...
        if program_mode == "anchor":
            ...
            query_prepared_genes, query_unliftable_genes = _anchor(*process_args)
            ...
            query_prepared_genes, query_unliftable_genes = _lift(*process_args)
        else:
            raise ValueError("program_mode is not one of ['anchor', 'lift']")

        total_unliftable_genes = len(query_unliftable_genes)
        ...
        query_unaligned_ranges = BaseGenomicRangesList([grange
                                                        ...
                                                        if grange.name in unaligned_range_names])
        total_unaligned_ranges = len(query_unaligned_ranges)
        lifted_unaligned_ranges = BaseGenomicRangesList([lifted_grange
                                                         for grange in query_unaligned_ranges
                                                         for lifted_grange in grange.relations['lifted']])

        pbar.update()

        query_exception_list = BaseGenomicRangesList(query_exception_ranges)

        with open(os.path.join(outdir, "query_unlifted.bed"), 'w') as outfile:
            query_unliftable_genes.to_bed6(outfile)

        with open(os.path.join(outdir, "query_exceptions.bed"), 'w') as outfile:
            query_exception_list.to_bed6(outfile)

        with open(os.path.join(outdir, "query_unaligned.bed"), 'w') as outfile:
            query_unaligned_ranges.to_bed6(outfile)

        with open(os.path.join(outdir, "subject_unaligned.bed"), 'w') as outfile:
            lifted_unaligned_ranges.to_bed6(outfile)

        pbar.update()
        stats_msg = "-----------------------\n" \
                    f"get_alignments stats:\n" \
                    f"Recieved {total_query_genes} transcripts.\n" \
                    f"Aligned {total_query_genes - total_unliftable_genes - total_exceptions} of them.\n" \
                    f"Estimated as unliftable {total_unliftable_genes} transcripts.\n" \
                    f"Estimated as unaligned {total_unaligned_ranges} transcripts.\n" \
                    f"Caught {total_exceptions} exceptions.\n" \
                    f"Distribution of amount of alignments:\n{simple_hist(alignment_counts)}\n" \
...
def _build(query_gene_alignments_filename, query_bg_filename, fitter, fdr, pval_threshold, gapopen, gapextend):
    ...
    try:
        ...
        for alignment in query_gene_alignments:
            ...
            if fdr:
                ...
                qvals = total_hsps * pvals / ranking(pvals)
                for hsp, qval in zip(alignment.HSPs, qvals):
                    hsp.pval = qval
                retain = qvals <= pval_threshold
                del pvals, qvals
                alignment.filter_by_bool_array(retain)
            else:
                for hsp, pval in zip(alignment.HSPs, pvals):
                    hsp.pval = pval
                del pvals
            ...
            try:
                ...
                subject_dropped.append(alignment.srange)
        if aligned:
            dropped_ranges = list()
        else:
            dropped_ranges = [query_dropped, subject_dropped]
        return query_orthologs, subject_orthologs, dropped_ranges
    except Exception as e:
...
def build_orthologs(alignments,
    ...
    with CmdProgressBar(cmd_hints,
        ...
        if len(orthologs) == 0:
            query_orthologs, subject_orthologs, dropped_ranges = [], [], []
        else:
            query_orthologs, subject_orthologs, dropped_ranges = zip(*orthologs)

        dist_found = [len(group) for group in query_orthologs]
        query_orthologs_bed12 = [ortholog[1]
                                 for group in query_orthologs
                                 for ortholog in group
                                 if ortholog]
        subject_orthologs_bed12 = [ortholog[1]
                                   for group in subject_orthologs
                                   for ortholog in group
                                   if ortholog]
        query_orthologs_total = [ortholog[0]
                                 for group in query_orthologs
                                 for ortholog in group
                                 if ortholog]
        subject_orthologs_total = [ortholog[0]
                                   for group in subject_orthologs
                                   for ortholog in group
                                   if ortholog]
        ...
        with open(query_output_bed12_filename, 'w') as outfile:
            for record in query_orthologs_bed12:
                outfile.write(record + "\n")
        with open(subject_output_bed12_filename, 'w') as outfile:
            for record in subject_orthologs_bed12:
                outfile.write(record + "\n")
        with open(query_output_total_filename, 'w') as outfile:
            for record in query_orthologs_total:
                outfile.write(record + "\n")
        with open(subject_output_total_filename, 'w') as outfile:
            for record in subject_orthologs_total:
                outfile.write(record + "\n")
        with open(query_dropped_filename, 'w') as outfile:
            query_dropped.to_bed6(outfile)
        with open(subject_dropped_filename, 'w') as outfile:
            subject_dropped.to_bed6(outfile)
        ...
        stats_msg = "-----------------------\n" \
                    ...
                    f"Reported all significant orthologs for each transcript.\n" \
                    "-----------------------"
        if isinstance(stats_filename, str):
            with open(stats_filename, 'a') as stats_file:
                pbar.write(stats_msg, file=stats_file)
...
def get_best_orthologs(query_orthologs,
                       ...
                       outfile_query_total,
                       outfile_subject,
                       outfile_subject_total):
    """A get_best_orthologs step of the pipeline."""
    query_filename = Path(query_orthologs)
    query_total_filename = Path(query_total)
    subject_filename = Path(subject_orthologs)
    subject_total_filename = Path(subject_total)
    value_name = value
    func_name = function
    query_output_filename = Path(outfile_query)
    query_total_output_filename = Path(outfile_query_total)
    subject_output_filename = Path(outfile_subject)
    subject_total_output_filename = Path(outfile_subject_total)
    ...
    with open(query_filename, 'r') as query_in, \
         open(query_total_filename, 'r') as query_total_in, \
         open(subject_filename, 'r') as subject_in, \
         open(subject_total_filename, 'r') as subject_total_in, \
         open(query_output_filename, 'w') as query_out, \
         open(query_total_output_filename, 'w') as query_total_out, \
         open(subject_output_filename, 'w') as subject_out, \
         open(subject_total_output_filename, 'w') as subject_total_out:
        purge = list()
        for query_line, query_total_line, subject_line, subject_total_line in zip(query_in,
                                                                                  query_total_in,
                                                                                  subject_in,
                                                                                  subject_total_in):
            if len(purge) == 0:
                purge.append((query_line, query_total_line, subject_line, subject_total_line))
                ...
                query_total_out.write(best_ortholog[1])
                subject_out.write(best_ortholog[2])
                subject_total_out.write(best_ortholog[3])
                purge = list()
                purge.append((query_line, query_total_line, subject_line, subject_total_line))
...
def run_pipeline(query_genes,
    ...
    get_best_orthologs(query_orthologs=query_orthologs,
                       query_total=query_total_orthologs,
                       subject_orthologs=subject_orthologs,
                       subject_total=subject_total_orthologs,
                       value=value,
                       function=function,
                       outfile_query=best_query_orthologs,
                       outfile_query_total=best_query_total_orthologs,
